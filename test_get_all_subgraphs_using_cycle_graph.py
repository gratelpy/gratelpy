...
def get_sensible_subgraphs_paths_in_cycles_using_cycle_graph(sc):
    ...
    for n in cycle_graph.nodes():
        if set(n) not in complete_subgraphs:
            complete_subgraphs.append((tuple(n)))
    ...
    for e in cycle_graph.edges():
        if set(e) not in complete_subgraphs:
            complete_subgraphs.append((tuple(e))) 
    k = 3
    ...
    for cycle_comb in complete_subgraphs:
        ...
        if (('s2', 'w1', 's1'), ('s1', 'w1', 's2')) in cycle_comb:
            print 'cycle_comb = '+str(cycle_comb)
            print 'len(cycle_comb) = '+str(len(cycle_comb))
            print 'substances in cycle_comb = '+str([path[0] for cycle in cycle_comb for path in cycle])
            print 'unused_edges = '+str(unused_edges_per_substance)
...
#    for sg in mixed_edges_cycles_subgraphs:
#        print sg
