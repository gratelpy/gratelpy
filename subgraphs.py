...
# ALL OF CODE BELOW REPLACED WITH CALL TO get_all_cliques in graphs.py    
#     # collect all complete subgraphs (cliques) in complement of cycle graph
#     complete_subgraphs = []

#     # every node by itself is a complete subgraph
#     for n in cycle_graph.nodes():
# #        if set(n) not in complete_subgraphs: # this check is redundant since every node appears exactly once in cycle_graph anyways
#             #print [n]
#         complete_subgraphs.append([n])

#     # at this point complete_subgraphs is of the format
#     # [ [cycle_1], [cycle_2], ..., [cycle_n]]
#     # where each cycle_i object has format
#     # ((path_1), (path_2), ..., (path_n_that_links_back_to_path_1))

#     # every edge in cycle graph is a complete subgraph (of cardinality 2)
#     for cycle_graph_edge in cycle_graph.edges():
#         # traversing all edges of cycle_graph, we visit each edge exactly once
#         # so there is no need to check if an edge is already in complete_subgraphs before adding it to that list
# #        if set([list([cycle for edge_endpoint in e for cycle in edge_endpoint])]) not in complete_subgraphs:
# #        if set(tuple([cycle for edge_endpoint in e for cycle in edge_endpoint])) not in complete_subgraphs: # this check is redundant
# #        complete_subgraphs.append([tuple([cycle for edge_endpoint in e for cycle in edge_endpoint])])
#         complete_subgraphs.append([cycle_graph_edge_endpoint for cycle_graph_edge_endpoint in cycle_graph_edge])

#     # complete_subgraphs now looks as follows
#     # [ [cycle_1], [cycle_2], ..., [cycle_n], edge_1, edge_2, ..., edge_m  ]
#     # where each edge_i is
#     # [cycle_k, cycle_l] ... where cycle_k and cycle_l share an edge in the (complement of the) cycle graph
#     # i.e. complete_subgraphs is
#     # [ [cycle_1], [cycle_2], ..., [cycle_n], [cycle_k1, cycle_k2], ..., [cycle_kn, cycle_km] ]

#     # add complete subgraphs of cardinality 3 and greater
#     k = 3
...
#                 complete_subgraphs.append(list(k_clique))

#         #DEBUG
#         print 'k-clique-community with k = '+str(k)
#         for k_clique in k_cliques:
#             print k_clique
#         #DEBUG END
...
#     #DEBUG
#     print 'all cliques'
#     all_cliques = networkx_modified.get_all_cliques(cycle_graph)
...
#     for cycle_1 in cycles_in_path_graph:
#         for cycle_2 in cycles_in_path_graph:
#             if cycle_1 is not cycle_2 and any(edge_cycle_1[0] in [e[0] for e in cycle_2] for edge_cycle_1 in cycle_1):
#                 cycle_graph.add_edge(tuple(cycle_1),tuple(cycle_2)) # nodes must be immutable objects
...
#     # all-edges subgraphs
#     # note that this method of creating all-edges subgraphs makes sure that every substance is the beginning of exactly one edge
#     # however, it does not ensure that the reaction nodes are used exactly as often as they occur in the fragment (multiplicity of reaction nodes isn't respected)
#     all_edges = [sc[key]['edges'] for key in sc.keys()]
#     all_edges_subgraphs = it.product(*all_edges)
...
#         # note that if cycle_comb already uses all substances as beginning of exactly one path then unused_edges_per_substances is empty and the generated subgraph is an all-cycles subgraph
#         for edges_combination in it.product(*unused_edges_per_substance):
#             mixed_edges_cycles_subgraphs.append([edge for edge in edges_combination]+[cycle for cycle in cycle_comb])
